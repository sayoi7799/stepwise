...
第 1 次尝试：覆盖率 78.2% → 偏难，加严约束重试
第 2 次尝试：覆盖率 87.8% → 偏难，加严约束重试
第 3 次尝试：覆盖率 84.0% → 偏难，加严约束重试

—— 最终交付给学习者的解释 ——
1. 在这句话里，“意思”指这句话很有趣，让人喜欢。
2. 这个电影很有意思，我很喜欢。
```
...
`moonbitlang/async`）；拼报文、读响应、切 SSE 流都是我们自己的纯函数，
有测试精确固定。想先看看要发出去什么，用 `prompt` 命令，它完全离线。

### 闸门是反馈，不只是拦截

上面那个例子里，三轮都没过门槛——但这不是失败，而是这个项目最核心的机制
在起作用。两个设计让「重试」真的有用：

**一、审计结果会回灌给模型。** 只说「上次太难了」没有信息量，模型不知道该
改什么。我们手上有确切数据——审计能算出**具体哪几个词超纲**——所以重试时
直接点名：

```
注意：上一次的解释对学习者太难了。下面这些词超出了 HSK 2，
请全部换掉，改用更简单的说法：让人、好玩、有趣。
```

**二、被解释的词本身排除在生词之外。** 解释里必然出现被解释的那个词，
它本来就是学习者不认识的（不然也不用解释）。不排除它，任何解释都会被
判成偏难，模型再怎么改也过不了门槛——度量会一直说谎。

第一条有实测数据：换成点名之后，同一句的重试结果从中位数 84% 提到 92%
（`79.3 → 92.3`、`78.2 → 87.8` 这样的轨迹），因为模型终于知道要换掉什么。
